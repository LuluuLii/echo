...
│                    Echo Mobile                       │
├──────────┬──────────┬──────────┬────────────────────┤
│  Today   │  Studio  │ Insights │      Library       │
└──────────┴──────────┴──────────┴────────────────────┘
```

- **Today**: 每日照片激活，被动触发
- **Studio**: 主动练习与创作的空间 — 对话练习 (Dialog) 或沉浸式写作 (Creation)
- **Insights**: 成长追踪，词汇、表达、主题的积累与进步
- **Library**: 素材库，照片 + 截图，自动聚类
...
Studio 是用户主动表达的空间，对应 Web 端的 Creative Studio。支持两种模式：

- **Dialog Mode（对话练习）**: 围绕一张照片/话题，与 Echo 进行自然对话
- **Creation Mode（沉浸式创作）**: 自由写作，Echo 仅在用户请求时介入

### 4.1 入口

从多个路径进入 Studio：

1. **Today → Start Echo**: 从今日 Activation Card 进入（默认 Dialog Mode）
2. **Studio Tab → 选择照片/话题**: 主动选择素材开始练习
3. **Studio Tab → Quick Write**: 直接进入 Creation Mode 自由写作
4. **Library → 某张照片 → Practice**: 针对特定照片练习
...
│  ┌─────────────────────────────┐   │
│  │  ✍️ Quick Write              │   │
│  │  Start a free-form writing   │   │
│  │  session on any topic        │   │
│  └─────────────────────────────┘   │
│                                     │
│  ─── Pick a Photo to Practice ───  │
│                                     │
│  Recent Photos                      │
│  ┌────┐ ┌────┐ ┌────┐ ┌────┐      │
│  │    │ │    │ │    │ │    │      │
...
**Quick Write** 进入 Creation Mode：
- 可选输入话题或直接开始写
- 可选关联照片作为灵感
- Echo 不主动回复，用户按需请求 Feedback / Hints / Continue
...
| Studio Session 对话 | 文字输入、Echo Companion 回应、Context 面板 |
