...
| 内容类型 | 是否持久化 | 说明 |
|----------|------------|------|
| **原始素材 (Raw Materials)** | ✅ 保存 | 用户的积累，是表达的原料 |
| **激活卡 (Activation Card)** | ❌ 不保存 | 当下生成，用于邀请表达，阅后即焚 |
| **表达输出 (Echo Output)** | ✅ 保存 | 用户真正说出来、整理出来的作品 |

**激活卡是「邀请函」，不是「作品」。** 它的价值在于唤醒表达欲望的那一刻，而非被收藏。真正有价值的是用户在激活后产出的表达——那才是属于用户自己的语言作品。

```
素材积累 ──→ 召回 ──→ 激活卡(临时) ──→ 表达输出 ──→ 作品保存
   ↑                      ↓                         │
   │                   阅后即焚                      │
   └─────────────────────────────────────────────────┘
                    持续积累
